...
| `gateway` | `src/api/Dockerfile` | **8080** | `/health/live` | **Entrada única da aplicação**: JWT, rate limiting por tenant/plano, CORS, headers de segurança; round-robin entre as réplicas do Consolidado com health checks ativo (2 s; uma falha retira a réplica) e passivo, timeout de conexão de 1 s e retentativa de leituras em outra réplica. Ambiente `Docker` (destinos pelos nomes dos serviços) |
